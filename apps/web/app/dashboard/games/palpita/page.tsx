...
export default function PalpitaReportPage() {
        ...
        const init = async () => {
            try {
                ...
                if (gamesRes.ok) {
                    ...
                    // Filter specifically for Palpita AI
                    const game = games.find((g: any) => g.type === "PAIPITA_AI")
    ...
    // Format guesses (c, e, f) nicely
    const renderGuesses = (numbers: string[]) => {
        if (!numbers || !Array.isArray(numbers)) return "-"
        // Assuming numbers are ["C", "E", "F", ...]
        // Limit display to a few to prevent table overflow, or show compact string
        return (
            <div className="flex flex-wrap gap-1 max-w-[200px]">
                {numbers.map((n, i) => (
                    <span key={i} className={`
                        w-5 h-5 flex items-center justify-center text-[10px] font-bold rounded
                        ${n === 'H' || n === 'C' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' :
                            n === 'D' || n === 'E' ? 'bg-blue-100 text-blue-700 border border-blue-200' :
                                n === 'A' || n === 'F' ? 'bg-red-100 text-red-700 border border-red-200' :
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-muted/30 hover:bg-muted/40 border-b border-border/60">
                                    <TableHead className="w-[180px] cursor-pointer hover:text-emerald-500 transition-colors" onClick={() => handleSort('createdAt')}>
                                ...
                                {loading ? (
                                    ...
                                    sortedTickets.map((ticket, index) => {
                                        return (
                                            <TableRow
                                                key={ticket.id}
                                                className={`transition-colors hover:bg-muted/30 cursor-pointer ${index % 2 === 0 ? 'bg-transparent' : 'bg-muted/10'}`}
                                                ...
                                                <TableCell>
                                                    <div className="flex items-center gap-2">
                                                        <span className="font-mono text-xs font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded">
                                                            {ticket.series ? `#${ticket.series.toString().padStart(4, '0')}` : '-'}
                                                        </span>
                                                    </div>
                                                </TableCell>
                                                <TableCell>
                                                ...
                                                <TableCell className="text-center">
                                                    {(() => {
                                                        ...
                                                        const statusConfig: Record<string, { label: string, color: string, icon: any }> = {
                                                            ...
                                                            'LOST': { label: 'Não Premiado', color: 'bg-slate-500/10 text-slate-500 border-slate-500/20', icon: AlertCircle },
